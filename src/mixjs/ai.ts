const summary =  `JS-AI组件，一个可嵌入事件流程中的JavaScript脚本组件，支持各类三方库引用。
何时使用：只有当需要整合输入、数据转换处理、复杂计算、处理流程分支、输出自定义内容等场景时，建议使用此组件编写代码来实现。`
...
export default {
  ...
  prompts: {
    summary2: `JS计算，一个可嵌入事件流程中的JavaScript脚本组件，允许自定义JavaScript代码来处理和转换输入数据，灵活实现业务逻辑，控制流程分支
只有当需要整合输入、数据转换处理、复杂计算、处理流程分支、输出自定义内容等场景时，才允许使用该组件`,
    usage2: `JS计算支持动态添加多个输入和输出端口，可实现复杂的数据整合，控制流程分支等功能。
