...
export default {
  ...
  prompts: {
    summary: `AI组件，一个支持自定义编写代码的React组件，支持各类三方库引用。
何时使用：在基础或者现有组件无法满足需求时，建议使用此组件编写代码来实现。建议推荐使用。
`,
    usage: `AI组件，一个支持自定义编写代码的React组件，支持各类三方库引用。
何时使用：在基础或者现有组件无法满足需求时，建议使用此组件编写代码来实现。

如何使用：
搭建使用时，无需提供代码，只需配置需求文档即可
需求格式为一句话的markdown，能清楚定义当前组件的边界即可，不要过于详细地描述内容，特别是具体的组件比如图片等
比如：
- "完成顶部导航栏区域的开发，白色背景，内容为左侧logo、公告 和 右侧的用户区域"
- "完成图表区域的开发，白色圆角卡片，内容为右上角的筛选和中间的折线图"。
`,
  },
  editors: [
    {
